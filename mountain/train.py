...
CRITIC_LR_RATE = 1e-3
NUM_AGENTS = 12
TRAIN_SEQ_LEN = 9000  # take as a train batch
TRAIN_EPOCH = 1000000
...
def pareto_rules(agent_results):
    r_0, pos_0 = agent_results[0]
    r_1, pos_1 = agent_results[1]
    _tmp = [-1., -1.]
    if np.abs(r_0 - r_1) <= 1e-6 and np.abs(pos_0 - pos_1) <= 1e-6:
        _tmp = [0., 0.]
        return _tmp
    elif np.abs(r_0 - r_1) > 1e-6:
        _tmp[np.argmax([r_0, r_1])] = 1.
    else:
        _tmp[np.argmin([pos_0, pos_1])] = 1.
    return _tmp
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open('elo.txt', 'w') as test_log_file:
        ...
        for epoch in range(TRAIN_EPOCH):
            print(str(epoch), '/' + str(TRAIN_EPOCH))
    ...
    with tf.Session() as sess, open(SUMMARY_DIR + '/log_agent_' + str(agent_id), 'w') as log_file:
        ...
        for epoch in range(TRAIN_EPOCH):
            ...
            for i in range(2):
                ...
                tmp_buffer.append(
                    [s_batch, a_batch])
                tmp_agent_results.append([reward, pos])
